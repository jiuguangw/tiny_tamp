...
def within_velocity_limits(position_curve, max_v=None, **kwargs):
    ...
    return all(
        np.less_equal(np.absolute(velocity_curve(t)), max_v).all() for t in extrema
    )
...
def smooth_curve(
    ...
    for iteration in irange(num):
        ...
        t1, t2 = np.random.uniform(
            times[0], times[-1], 2
        )  # TODO: sample based on position
        ...
        if (
            min_t >= current_t
        ):  # TODO: also limit the distance/duration between these two points
            continue
        ...
        if refit:
            ...
            if cubic:
                ...
                new_curve = CubicHermiteSpline(
                    new_times, new_positions, dydx=new_velocities
                )
            else:
            ...
            if (
                spline_duration(new_curve) >= spline_duration(curve)
            ) or not check_spline(new_curve, v_max, a_max):
                continue
        print(
...
def plot_curve(positions_curve, derivative=0, dt=1e-3):
    ...
    for i, coords in enumerate(zip(*[curve(t) for t in times])):
        plt.plot(
            times, coords, color=colors[i], label="x[{}]".format(i)
        )  # , marker='o-')
